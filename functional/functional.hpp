...
template< >
struct Functor<Data::List::basic_list>
{
    template< typename a, typename b >
    static Data::List::basic_list<b> fmap(b(*function)(const a&),
                                          const Data::List::basic_list<a>& list)
    {
        if (typeid(list) != typeid(Data::List::Nil<a>))
        {
            return Data::List::concat(function(list.value), fmap(list->next));
        }
        else
        {
            return list;
